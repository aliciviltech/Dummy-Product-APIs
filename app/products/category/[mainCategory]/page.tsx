...
export default async function CategoryProduct({ params }: { params: Promise<{ mainCategory: string }> }) {

    const paramData = await params;
    const { mainCategory } = paramData

    const products = AllProducts.filter((item) => item.mainCategory === mainCategory)

    return (
        <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
            <h1 className="w-full text-3xl px-6 py-4 text-center select-none font-bold font-sans text-cyan-500 mb-10">{mainCategory.slice(0,1).toUpperCase()+mainCategory.slice(1)} Products</h1>
